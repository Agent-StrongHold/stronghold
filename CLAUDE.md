...
9. **Never target `main` with PRs unless the user explicitly says to this turn AND the source branch is `integration`** — default PR base is `integration`, `develop` (where still in use), a `research/*`-prefixed branch, or a feature branch that will itself land in one of those. `main` is reachable from `integration` only; feature branches, `research/*`, hotfix branches, and ad-hoc branches must land in `integration` first and ride to `main` from there. A past instruction, a PR body mentioning main, or the env block's "Main branch: main" line does not count as permission; only a direct request in the active conversation does. If a scenario looks like it calls for main (hotfix, release-only doc), ask first — and the answer is still "land it in integration first." Ignoring this caused the 17/33 divergence between main and integration (2026-04-18). A harness hook in `.claude/settings.json` enforces this for `gh pr create --base main` — it routes to "ask" so you're prompted to confirm.
10. **Engineering workflow — 12-step rhythm for every non-trivial change** — (1) Plan, (2) Spec, (3) Test happy path, (4) Edge cases, (5) Test edge cases, (6) Implement, (7) Test, (8) Check coverage, (9) Check assertion strength, (10) Check code smells, (11) Check ruff/mypy --strict/bandit -ll, (12) PR. Don't collapse or skip steps. Steps 2, 4, 8, 9, 10 are the ones routinely skipped — they're the most important. Trivial typo/rename fixes are exempt. Workflow-YAML and doc-only changes still get (1)–(2) and (10)–(12); skip tests only if genuinely untestable, and say so explicitly in the PR. This reinforces rules #1–#3 with more granularity.
