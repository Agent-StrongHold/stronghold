...
to choose one for the reference deployment, and that choice shapes
...
or OpenShift. The runtime we choose for the reference deployment
should be one whose security defaults and operational patterns prepare the
chart to ship cleanly to those environments.

## Decision

**The Stronghold reference deployment runs on OKD single-node
...
4. **Apache 2.0 community licensing.** No Red Hat subscription required for
   the dev cluster. No pull-secret renewal cycle. No license cost. Fully open.
...
- The development cluster is a real Kubernetes runtime, not a CI tool. Cluster
  upgrades follow OKD's upgrade flow (every 4-6 months, ~30 min, mostly
  automated).
- The engineer running the dev cluster learns OpenShift conventions (`oc` CLI,
...
- Rejected as the reference deployment, but acceptable as a Tier-2 supported
  distro for customers. Reasoning: k3s is a fine production runtime, but
  using it as the reference means we never test against the OpenShift surface
  ...
  break only when a customer reports it. The whole point of the dev cluster is
  to be the early-warning system for chart compatibility issues.

**C) RKE2 on a dedicated VM.**

- Rejected as the reference deployment for the same reason as k3s: it's an
...
- Rejected for the dev cluster: requires a Red Hat subscription, which adds a
  ...
  secret. None of those differences matter for a single-operator development
  deployment. We document OCP as a Tier-1 supported customer distro in
  ADR-K8S-007, just not the dev cluster choice.

**E) MicroShift (edge-stripped OpenShift) for the dev cluster and full CRC for
...
- Rejected: MicroShift drops the Web Console, OperatorHub, ImageStreams,
  ...
  compatibility with full OpenShift. We want the dev cluster to be
  ...
  for development reference deployments. (See ADR-K8S-012 for the related
...
- Rejected for the dev cluster: ongoing cloud cost for what is fundamentally a
...
- The engineer running the dev cluster develops OpenShift muscle memory that
...
- OKD is heavier than k3s or RKE2: ~16GB RAM minimum for the SNO node,
  ...
  cluster operators to monitor. Acceptable on the dev cluster hardware (i9-
...
- We accept higher resource usage and a steeper learning curve in exchange
  for catching enterprise compatibility issues on the dev cluster instead of
