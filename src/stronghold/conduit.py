    ...
    async def route_request(
        ...
        *,
        ...
        providers_cfg: dict[str, ProviderConfig] = {
            k: (
                ProviderConfig(**{fk: fv for fk, fv in v.items() if fk in _prov_fields})  # type: ignore[arg-type]
        ...
        with trace.span("conduit.route") as rs:
            ...
            models: dict[str, ModelConfig] = {
                k: (
                    ModelConfig(**{fk: fv for fk, fv in v.items() if fk in _model_fields})  # type: ignore[arg-type]
                    if isinstance(v, dict)
                    else v
                )
