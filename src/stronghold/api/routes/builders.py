    ...
    try:
        ...
        pr_result = await td.execute(
            "github",
            {
                ...
                "head": branch,
                # Stronghold's long-lived staging branch is `integration`
                # (per project convention in CLAUDE.md). Mason PRs land
                # on integration first, where they get lint + tier-2
                # tests. The release.yml 95% coverage gate only applies
                # to the eventual integration → main promotion, not to
                # the per-issue Mason PRs.
                "base": "integration",
                "body": (
                    f"## Issue #{issue_num}\n\n"
...
async def _create_pr_after_success(
    ...
    pr_result = await tool_dispatcher.execute(
        "github",
        {
            ...
            "head": branch,
            # See the rationale next to the other create_pr call:
            # Mason PRs target integration, not main.
            "base": "integration",
