...
def _render_text(
    ...
    try:
        ...
        font = ImageFont.truetype(font_path if font_path else "DejaVuSans-Bold.ttf", font_size)
    except OSError:
...
async def execute_canvas(  # noqa: C901, PLR0912
    ...
    *,
    ...
    if action == "composite":
        ...
        if layers:
            for spec in layers:
                ...
                if layer is None:
                    continue
                assert layer is not None  # for mypy narrowing
                if "x" in spec:
                    layer.x = spec["x"]
                if "y" in spec:
                    layer.y = spec["y"]
    ...
    if action == "upload":
        ...
        img: Any = Image.open(io.BytesIO(img_bytes)).convert("RGBA")
