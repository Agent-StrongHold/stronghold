...
class TestInboundWebhookAPI:
    ...
    async def test_register_webhook_returns_all_fields(
        self, webhook_store: InMemoryWebhookStore
    ) -> None:
        """Register returns id, name, secret, source, agent, enabled."""
        app = _build_test_app(webhook_store)
        async with AsyncClient(
            transport=ASGITransport(app=app), base_url="http://test"
        ) as client:
        ...
        assert resp.status_code == 201
        data = resp.json()
        assert data["name"] == "Deploy Hook"
        assert data["source"] == "ci"
        assert data["agent"] == "ranger"
