"""Tests for trigger handlers not covered by test_new_modules_2.

  - rate_limit_eviction: the "evicted > 0" debug log branch
  - security_rescan:     the "not verdict.clean" warning branch
  - post_tool_learning:  failure-with-tool_name and success branches
...
class TestRlhfFeedback:
    ...
    async def test_lazy_initialisation_and_processing(
        ...
        result = await action(Event(name="pr.reviewed", data={"review_result": review}))
...
class TestMasonPrReview:
    ...
    async def test_skip_when_no_pr_number(self) -> None:
        ...
        result = await action(Event(name="mason.pr_review_requested", data={}))
        assert result == {"skipped": True}
