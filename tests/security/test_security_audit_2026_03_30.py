...
class TestHighQuotaDashboardXSS:
    ...
    def test_h9_csp_allows_unsafe_inline(self) -> None:
        """H9 vulnerability surface removed with the Turing UI rewrite.

        Prior form: asserted that ``/dashboard/quota`` emitted a CSP with
        ``'unsafe-inline'`` in script-src, documenting the XSS path via
        ``marked.parse`` → ``innerHTML`` on that page. quota.html has been
        deleted in the phosphor-noir dashboard, so there is no quota page
        to harden. The surviving dashboard pages (chat, notebook, blog,
        profile, memory, hub, canvas) get their CSP audited under the new
        ...
        assert not quota_html.exists(), (
            "quota.html is back — re-enable the H9 XSS regression accordingly."
        )
