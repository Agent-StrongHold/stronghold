...
- **1000Hz Reactor** — Core-process event backbone (not a separate agent) that unifies all proactive behavior into one evaluation loop. Four typed trigger modes (event, interval, time, state), per-trigger circuit breakers, blocking triggers for gates (≤1ms), async dispatch for side effects. Runs in-process alongside the API/router/classifier — the heavy work happens in spun-out containers and async tasks. No comparable system found in frameworks or in our literature review.
