...
- **1000Hz Reactor** — Pure event-driven tick loop (same pattern as a game loop) that unifies all proactive behavior into one deterministic evaluation cycle. Single-threaded condition evaluation guarantees consistent ordering with no race conditions — the heavy async work it dispatches can be parallel, but the loop itself is deterministic. Four typed trigger modes (event, interval, time, state), per-trigger circuit breakers, blocking gates (≤1ms), async dispatch for side effects. Runs in core process space alongside the API/router/classifier. 0.46% of one core for the guarantee that every trigger fires regardless of emitter health. No comparable system found in frameworks or in our literature review.
